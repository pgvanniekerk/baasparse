...
| ASM-3 | A **shared file-storage area** (e.g. NFS/clustered FS) is provided, reachable by all servers, with adequate performance and correct **atomic-rename** semantics. File **claiming** is done in PostgreSQL (`BR-HA-003`), **not** via shared-FS locks, so unreliable NFS lock semantics do not affect correctness. **New-file detection on the shared area is by directory scan, not `inotify`** — `inotify` does not report files written by other hosts onto a networked mount (`BR-COL-012`). | Claim coordination is robust; detection latency is bounded by scan interval, not by `inotify`. Partial-read risk if atomic-rename semantics are wrong. |
...
| ASM-18 | **Whole-site disaster recovery of the shared file area** is provided by the **storage/platform layer** (storage-array or filesystem replication to the DR site), sized to an **RPO aligned with the PostgreSQL DR standby** (`BR-HA-006`). The engine deliberately keeps **no record content in the database** (`BR-NFR-009`), so on a site-loss failover the promoted DR database's file references are only valid if the file area was replicated alongside it; the engine's role is limited to the **startup DB↔disk reconciliation** (`BR-NFR-017`) that *detects* divergence. File-area DR is therefore treated like database HA (Patroni/repmgr) and shared-FS HA (`R12`): a **platform responsibility the engine depends on, not one it implements**. | If the file area is **not** DR-replicated, a whole-site failover loses all on-disk record content (suspense, in-progress, un-archived output) irrecoverably — the engine cannot mitigate this alone; it is called out here so the platform provisions it. |
