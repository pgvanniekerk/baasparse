...
| **Backfill windowing mode** | The collation behaviour used **during a declared catch-up**: because the wall-clock grace-timeout misfires under a burst of historical records, windows complete on **event-intrinsic** triggers where available, else are **held until the key/source backlog drains** and then flushed — never closed on wall-clock (`BR-COR-007`, `BR-OPS-013`). |
| **Deterministic output-record identity** | A replay-stable, unique-per-logical-record identity derived from source lineage (file UID + record position, or a business key; for aggregates, group key + window identity), used as the RDBMS upsert/delivery-dedup key so retries/recovery/re-send/replay never drop or duplicate (`BR-DST-013/018`). |
| **Bounded store-and-forward** | Store-and-forward (holding output for a down destination) with a **configurable max spool size/age** and a defined **overflow policy** (pause intake, or divert to suspense) so one dead consumer cannot fill the shared disk; discarding output on overflow is never allowed (`BR-DST-017`). |
| **Suspense escrow** | An optional, size-/retention-bounded copy of an **individual** suspended record's bytes, so the rest of its (large) source file can be archived while the record awaits correction — a deliberate narrow exception to no-content-off-stream (`BR-ERR-011`); default is whole-file retention (`BR-ERR-008`). |
| **Audit external anchor** | A periodic emission of the append-only audit **chain-head hash** to a location outside the mutable database (WORM/restricted log, export, alert channel), so a full-table rewrite of the hash chain is detectable against an independently-held value (`BR-AUD-004`). |
| **Window version-pinning** | Stamping each collation window, at open, with the **pipeline-config version that governs it**; appends/completion/emit all follow the stamped version, a publish never mutates open windows, and no window mixes two versions' semantics (`BR-COR-011`). |
| **Adjustment identity** | The deterministic identity of an adjustment/**delta** emission: the original window identity + a monotonic adjustment sequence, plus a reference to the identity it adjusts — so deltas never overwrite the original aggregate at an idempotent target, while a **verified full replacement** re-uses the original identity (`BR-COR-012`, `BR-DST-018`). |
| **Divert holding area** | The bounded on-disk area where output for a dead destination is materialised under the *divert* overflow policy; the endpoint's Delivery Record is marked `diverted` — terminal for the done-lifecycle, re-sendable, reconciled as not-delivered — keeping in-progress bounded (`BR-DST-017`). |
| **State-aware alerting** | Suppressing/downgrading alerts whose firing condition is *caused by* a declared engine/operator state (pause, hold, catch-up, fail-closed), replaced by a single alarm for the state itself; suppression is visible (`BR-OPS-017`). |
| **Output control records** | Configurable header/trailer records in output files (record count, optional checksum/sequence/timestamps) giving downstream the same per-file completeness contract the engine demands of its sources (`BR-DST-020`). |
| **Backdated correction** | Publishing a config/reference-data version whose **validity covers a past period**, superseding the defective version *for that period* within the temporal model (`BR-CFG-009`) — so effective-dated selection ("effective at event time, per the latest published config") lets suspense reprocessing pick up the fix (`BR-CFG-014`). |
| **Retransmission horizon** | The longest delay after which a source may still legitimately re-deliver the same records; the dedup retention window is sized to cover it (`BR-DUP-002/006`). |
| **Staging quota** | A per-source bound on **fetched-but-not-yet-processed** files/bytes; remote fetch pauses at the bound (and whenever intake is paused) so the input side cannot fill the shared disk (`BR-RMT-013`). |
| **Reference-data readiness precondition** | An optional per-pipeline declaration that required reference data must be present/fresh before intake runs; if unmet, intake is held and alerted instead of flooding suspense with on-miss failures (`BR-ENR-006`). |
...
| **Output sequencing / file UID** | A **monotonic, contiguous-at-commit (gap-detectable)** identifier per scanned file, echoed into output files so downstream can detect a missing file; allocated transactionally, so a gap is a real missing-file signal, not allocator noise (`BR-COL-005`, `BR-DST-011`). |
...
| **Canonical record** | The engine's format-independent internal representation of a decoded record (typed fields, explicit units); the "common format" stored for collation and consumed by the **v1 RDBMS load target** (`BR-DEC-009`, `BR-COR-006`, `BR-DST-007`). |
...
| **Crypto-shredding** | Rendering a data subject unidentifiable by **destroying their token/key mapping** rather than rewriting stores — the mechanism by which data-subject erasure (`BR-CMP-005`) coexists with the append-only, tamper-evident audit trail (`BR-AUD-004`). |
| **Fail-closed (state-store loss)** | The engine's behaviour when no PostgreSQL primary is reachable: stop claiming files, quiesce at checkpoints, never deliver output whose delivery can't be recorded; degraded state stays visible via DB-independent health endpoints (`BR-NFR-019`). |
| **Management-plane entry point** | The platform-provided VIP / load balancer / reverse proxy through which clients reach the GUI/API of any instance; the engine's side is being instance-agnostic with sessions/tokens in shared PostgreSQL (`BR-HA-012`, `ASM-19`). |
